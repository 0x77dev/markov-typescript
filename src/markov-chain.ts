...
export class MarkovChain<T> {
  ...
  protected readonly terminals: MarkovTerminalItems<
    T
  > = new MarkovTerminalItems<T>();
  ...
  private learnWithPrevious(previous: ChainState<T>, next: T): void {
    const weights = this.items.getOrCreateValue(
      previous,
      () => new WeightedDictionary<T>(this.toStrFunction)
    );
  ...
  walkWithPrevious(previous: T[]): T[] {
    ...
    while (true) {
      ...
      const value = utils.randomNumberBetween(
        1,
